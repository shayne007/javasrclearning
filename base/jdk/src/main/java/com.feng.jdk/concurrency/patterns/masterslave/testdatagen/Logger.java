...
public class Logger {
    ...
    // 接口日志输出目标目录
    public static final String LOG_FILE_BASE_DIR = System.getProperty("java.io.tmpdir") + "/tps/";
    ...
    public Logger() {
        ...
        try {
            ...
            baseDir.mkdirs();
            cachedPwr = new PrintWriter(new FileWriter(LOG_FILE_BASE_DIR + retrieveLogFileName(now)), true);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void printLog(LogEntry entry) {
        ...
        pwr.print(Tools.getUTCTimeStamp(Long.valueOf(entry.timeStamp), TIME_STAMP_FORMAT));
        pwr.print('|');
    ...
    private String retrieveLogFileName(long timeStamp) {
        ...
        return "ESB_interface_" + Tools.getUTCTimeStamp(timeStamp, TIME_STAMP_FORMAT1) + ".log";
    }
