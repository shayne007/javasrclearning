...
import com.feng.jdk.concurrency.patterns.poolworker.ReEnqueueRejectedExecutionHandler;
...
public abstract class AsyncTask<V> {
    ...
    static {
        DEFAULT_EXECUTOR =
                new ThreadPoolExecutor(1, 1, 8, TimeUnit.HOURS, new LinkedBlockingQueue<Runnable>(), new ThreadFactory() {
                 ...
                 * 该RejectedExecutionHandler支持重试。 当任务被ThreadPoolExecutor拒绝时， 该RejectedExecutionHandler支持
                 * 重新将任务放入ThreadPoolExecutor 的工作队列（这意味着，此时客户端代码 需要等待ThreadPoolExecutor的队列非满）。
                 */
    ...
    /**
     * 对外（其子类）暴露的服务方法。 该类的子类需要定义一个比该方法命名更为具体的服务方法（如downloadLargeFile）。 该命名具体的服务方法（如downloadLargeFile）可直接调用该方法。
     *
